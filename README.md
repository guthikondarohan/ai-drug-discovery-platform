...
### **Features:**
- 💬 **ChatGPT Dark Theme** - Sleek, modern interface
- 🔮 **Real-time Predictions** - Enter SMILES notation for instant molecular activity predictions
- 📊 **Multi-File Upload** - Upload multiple CSV files at once (molecular or general data)
- 🧪 **Smart Detection** - Automatically detects molecular vs. general datasets
- 📈 **Interactive Visualizations** - Beautiful Plotly charts and confidence gauges
- 💊 **13 RDKit Descriptors** - Full chemical property analysis
- 🤖 **AI Assistant** - Ask questions about molecular properties, LogP, SMILES notation, etc.
...
- **`src/`** — Core Python modules:
  - `data_preprocessing.py` — Molecular featurization with RDKit (13 descriptors)
  - `embeddings.py` — Molecular fingerprints and text embeddings
  - `models_fixed.py` — Neural network architectures
  - `train.py` — Training loops with validation
  - `evaluate_fixed.py` — Evaluation metrics and ROC plots
  - `utils.py` — Utilities (seed setting, directory helpers)
  - `main.py` — CLI orchestration for full pipeline
- **`scripts/`** — Convenience wrappers (PowerShell and Bash)
- **`results/`** — Model outputs, metrics, plots
- **`app.py`** — Full-featured Streamlit dashboard
- **`chat_app.py`** — ChatGPT-style chat interface ⭐
- **`run_chat.ps1`** — Quick launcher for chat interface
...
- ✅ **13 RDKit Molecular Descriptors** - Full chemical property analysis
- ✅ **ChatGPT-Style Interface** - Modern, intuitive chat UI
- ✅ **Multi-File Upload** - Process multiple datasets simultaneously
- ✅ **Smart Dataset Detection** - Auto-detects molecular vs. general data
- ✅ **Real-Time Predictions** - Instant SMILES analysis
- ✅ **Interactive Visualizations** - Plotly charts with dark theme
- ✅ **AI Assistant** - Answer questions about chemistry and drug discovery
- ✅ **Export Capabilities** - Download results as CSV
- ✅ **Windows Compatible** - PowerShell scripts included
...
CCO
```
→ Instant prediction with confidence gauge and molecular properties
...
- Molecular data (with 'smiles' column) → Activity predictions
- General data → Dataset overview and preview
...
- `data/molecules_sample.csv` — 10 sample SMILES molecules with labels
- `data/clinical_text_sample.csv` — 3 sample clinical notes
